...
def render_ocr(root: Path, font_path: Path) -> tuple[list[dict[str, object]], list[dict[str, object]]]:
    ...
    texts = [
        ...
        ("ocr-traditional-clear-1", "traditional", "清晰掃描文字用於驗證文件轉換品質與字元順序。", 0.05),
        ("ocr-traditional-clear-2", "traditional", "離線處理應保持標題、段落、數字二零二六和標點。", 0.05),
        ("ocr-traditional-clear-3", "traditional", "安全邊界拒絕損壞輸入並傳回穩定診斷結果。", 0.05),
