...
case-fold-colliding IDs and paths, non-NFC or escaping paths, symlinks, size/hash drift, and files
that are present on only one side of the manifest/filesystem relationship.
...
environment after obtaining the font through the explicit manual target:
