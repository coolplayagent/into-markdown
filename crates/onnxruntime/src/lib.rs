...
mod tests {
    ...
    #[derive(Clone)]
    struct FixedResolver(ResolvedModel);

    impl into_markdown_ocr::ModelResolver for FixedResolver {
        fn resolve(
            &self,
            model_id: &str,
            _: &ExecutionContext,
        ) -> Result<ResolvedModel, ConversionError> {
            assert_eq!(model_id, "pp-ocrv6-tiny-recognizer-onnx");
            Ok(self.0.clone())
        }
    }
    ...
    fn official_ppocrv6_recognizer_meets_checked_corpus_cer() {
        let Some(repository) = option_env!("OCR_QUALITY_MODEL_REPOSITORY") else {
            return;
        };
        let runfiles = PathBuf::from(std::env::var_os("TEST_SRCDIR").unwrap());
        let model_path = runfiles
            .join(repository)
            .join("PP-OCRv6_tiny_rec_onnx_infer/inference.onnx")
            .canonicalize()
            .unwrap();
        let model_bytes = fs::read(&model_path).unwrap();
        assert_eq!(model_bytes.len(), 4_462_639);
        assert_eq!(
        ...
        #[cfg(not(unix))]
        let identity = "quality-runfile-hash-verified".to_owned();
        ...
        let recognizer = into_markdown_ocr::PpOcrTextRecognizer::new(
            tensor_runtime,
            &dictionary,
        ...
        for golden in goldens {
            ...
            crops.push(into_markdown_ocr::CropDescriptor {
                polygon: [
                    (0.0, 0.0),
                    ((width - 1) as f32, 0.0),
