...
mod linux {
    ...
    impl Seccomp {
        pub(super) fn prepare(allow_child_processes: bool) -> std::io::Result<Self> {
            ...
            if allow_child_processes {
                // Rust's fork/exec path uses a private Unix socketpair to report an exec error
                // to its parent. AF_UNIX socketpairs are local IPC and confer no network
