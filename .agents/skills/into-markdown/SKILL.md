...
3. Run conversion as `into-md <INPUT>` with an explicit output destination. There is no `convert` subcommand. Respect discovered user and project configuration unless the user requests an isolated diagnostic run.
...
- Treat OCR, speech, diarization, and legacy Office implementations as complete capability plugins. If one is unavailable, report the missing product capability and its stable diagnostic; do not expose or manage internal model bundles.
