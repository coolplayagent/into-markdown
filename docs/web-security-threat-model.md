...
原生文件选择和剪贴板读取由用户操作触发，沿用所有 API 的会话、Host 与 Origin 校验。文件路径仅在本机选择进程和服务间的私有管道传递，HTTP 只返回随机选择标识、名称和大小。文件正文由服务使用已打开的句柄流式复制；网页无法通过传入路径扩大读取范围。复制前后校验大小与修改时间，路径替换不会重定向已打开句柄。原生选择进程可被取消及服务关闭终止。

选择授权受单会话、数量和闲置过期限制；剪贴板图片快照还受单项及合计大小限制，移除和过期清理临时文件。原生剪贴板库在取得位图后检查尺寸，操作系统及库内部的初始位图分配仍属于本机原生输入的资源边界。网页文字、URL 和普通路径文本不会自动转换为文件授权。
