...
- DOC 解析 Word 97–2003 FIB 与 piece table；PPT 按 CurrentUser、UserEdit 与 persist authority
  ...
  Workbook IR 组装。
- CFB 默认入口以及 DOC、PPT 始终使用严格语义。调用方已选择 XLS 或 MSG 且错误策略为
  `best-effort` 时，reader 可忽略不会改变有效 stream 可寻址性或链所有权的冗余元数据；
  循环、越界、重叠、歧义、加密和资源超限在两种策略下都失败。
- 共享 CFB reader 在 strict 与 compatibility 两种入口都按已认证结构增量持有真实分配 lease；
  XLS 与 MSG 复用同一 compatibility 实现，不改变资源预算、链所有权或 DOC/PPT 行为。
