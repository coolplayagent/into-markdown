...
- Outlook MSG 的 CFB/OLE reader 对 DIFAT、FAT、miniFAT、directory sibling/child 图和每条
  ...
  closed。strict stream chain 必须与声明长度所需 sector 数完全一致；best-effort 仅复用共享
  CFB 的冗余元数据恢复，声明数据仍须完整可达且独占。MSG 的单个字符串/RTF 终止 NUL、
  空属性、未使用 padding 和显示名收件人恢复均带诊断；内部 NUL、缺失属性流仍被拒绝，
  空正文不触发低优先级正文回退，也不丢弃合法的主题或附件。
  CID 和 HTML 外链不触发网络；
...
- PDFium 仅从认证路径加载 authority 中精确 hash/size/ABI 的当前平台制品。Windows CLI 的默认路径
  由已经规范化的可执行文件位置推导；先认证可执行文件同级 portable 候选，仅在其不存在时认证
  `bin` 上一级的安装布局候选。显式 `PDFIUM_LIBRARY`、
  ...
  `PATH` 或动态加载器 fallback，也不联网。嵌入运行时 materializer 在受保护目录中完成逐文件认证
  后返回物理 canonical 路径，因此 macOS `/var -> /private/var` 系统别名不会被误当成未认证运行时，
  后续 pinned loader 仍只接收已认证快照。所有 native count/length、页、字符、
