...
本地命令输出用于确定性门禁的 Core tar 与三个 `.imp`。正式发布工作流先对两次未注入发布凭据的干净构建逐字节比较，再从已验证的同一构建产物生成 Developer ID 签名版本；Core 装入 DMG 后提交 Apple 公证并 stapling，三个插件分别以 ZIP 载体提交公证。最终发布 DMG、三个 `.imp` 及各自 SHA-256。
