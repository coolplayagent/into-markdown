...
Issue #270 depends on #268 for degraded, failed, and empty result semantics; #268 depends on #272.
The release order is therefore **#272 → #268 → #270**. This document defines converter-internal
boundaries only. It does not alter Core, Engine, result DTOs, or conversion outcomes.
...
| --- | --- | --- |
| `package/security.rs` | raw ZIP admission, canonical part names, duplicate/encryption/symlink/compression checks, bounded member reads, macro/active-part exclusion | XML semantics or result classification |
| `package/content_types.rs` | authoritative defaults/overrides and media-type lookup | filename-driven format decisions |
...
| `tests/` | package, QName, order, tables, fields, media, altChunk, strict, resource and corpus contracts | production helpers |
...
4. Nested payloads receive a child budget capped by remaining root input, memory, temporary-space,
   node, inline, asset, and nesting allowances. A child cannot create a fresh root budget.
...
6. The converter returns content plus scoped diagnostics. Empty/degraded/failed terminal semantics
   are integrated only after #272 and #268 land.

## Migration constraints

- No DOCX production file may grow beyond 900 lines; orchestration stays below 350 lines.
- Functions accept focused context structs instead of adding argument-count suppressions.
- Existing `too_many_lines` and `too_many_arguments` suppressions are removed as their functions
  move; no replacement suppressions are introduced.
...
## Integration point after dependencies land

After #272 and #268 merge, #270 can map converter evidence to the shared terminal contract:
