...
[0.0.5 全量报告](qa/evidence/release-0.0.5/README.md)冻结 `real-world-test-data` 的
2,285 个输入（1,979,990,949 字节），比较已发布 0.0.4、本次正式 0.0.5 构件的 OCR off/auto，
以及本地 MarkItDown 0.1.7、AnyDoc 0.2.4。报告区分非空输出、失败、质量代理、OCR 抽检、
资源与格式策略造成的兼容性变化；保留逐文件身份和测量脚本。该专项本地串行执行，不新增 CI。
...
- `tools/legacy-xls-public-quality.py` 可在本地或发布验收环境中使用仓库原创 `normal.xls` 与
  固定的 `xlrd 2.0.2` 逐单元格核对显示文本、数值/日期、sheet/row/cell 顺序和合并区；同一
  合法输入分别走 strict 与 best-effort，输出必须一致。配套 converter 测试执行 DOC/PPT 的
  损坏 CFB 对抗回归，证明 XLS 的兼容策略没有放宽其它旧 Office 格式。
