...
try {
  ...
  assert.equal(await page.locator('.current-task-link').count(), 1, 'Excel finishes independently');
  await small.click();
  ...
  if (batchCount > 0) {
    ...
    await page.locator('.convert-button').click();
    await page.waitForFunction(() => document.querySelectorAll('.current-batch-scroll li.succeeded').length === 100, null, { timeout: 180000 });
    await page.getByRole('button', { name: '下一页任务', exact: true }).click();
    const remaining = batchCount + 2 - 100;
    await page.waitForFunction(count => document.querySelectorAll('.current-batch-scroll li.succeeded').length === count, remaining, { timeout: 180000 });
