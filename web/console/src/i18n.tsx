...
const messages = {
  "zh-CN": {
    ...
    ocrRecognitionMemoryFailure: "此文件所需的图片文字识别超出识别内存额度，转换未完成。",
    ocrMemoryOmission: "部分图片的文字识别超出识别内存额度，正文、原图及其他识别结果已保留。",
    ocrOmissionDetailsUnavailable: "遗漏位置详情暂时无法加载，请下载诊断文件查看。", ocrOmissionPage: "页", ocrOmissionSlide: "幻灯片",
    diagnosticsPreviewUnavailable: "诊断预览不可用，请在详情与资源中下载完整诊断。", archiveExtractionRequired: "RAR 归档请先解压后再转换", unsupportedFormatFailure: "不支持这种文件格式", malformedInputFailure: "文件内容损坏或格式不正确", encryptedInputFailure: "文件已加密或受密码保护", resourceLimitFailure: "文件超出当前资源限制", ocrFailure: "文字识别失败", aiFailure: "AI 处理失败", networkFailure: "网络访问失败", ioFailure: "无法读取或写入文件", componentUnavailableFailure: "所需本地依赖尚未准备", timeoutFailure: "转换超时", recoveryFailure: "无法恢复此前的转换任务", internalFailure: "转换服务遇到内部错误", conversionFailedReason: "文件内容无法转换，请检查文件是否完整", invalidOptionsFailure: "转换设置无效", unreachableFailure: "无法连接本地转换服务", failureDetails: "失败详情",
