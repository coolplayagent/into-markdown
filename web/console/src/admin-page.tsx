...
const zh = {
  ...
  capabilitiesTitle: "转换能力", capabilitiesBody: "选择本地能力插件或远端 Provider。内置模型随插件安装，不需要单独管理。", installed: "已安装", notInstalled: "未安装", defaultModel: "默认模型", downloadOptions: "下载选项", insecure: "允许使用不安全的 HTTP 下载", capabilityRemoveConfirm: "确定卸载这个本地能力插件吗？远端 Provider 配置不会受影响。", currentSource: "当前来源", localPlugin: "本地插件", remoteProvider: "远端 Provider", off: "关闭", officeCapability: "旧版 Office", officeCapabilityBody: "转换 .doc、.xls 和 .ppt 文件", ocrCapability: "OCR", ocrCapabilityBody: "识别扫描 PDF 和图片中的文字", speechCapability: "语音", speechCapabilityBody: "分别选择转写和说话人识别来源", transcription: "转写", diarization: "说话人识别", useSource: "使用此来源", repair: "修复", capabilityInstallSuccess: "能力插件已安装", capabilitySourceSuccess: "能力来源已更新", capabilityVerifySuccess: "能力插件验证通过", capabilityRemoveSuccess: "能力插件已卸载", localOnly: "仅支持本地插件",
...
export function AdminPage({ api, section }: { api: ApiClient; section: AdminSection }) {
  ...
  useEffect(() => { setNotice(""); setActionFeedback(null); }, [section]);
  useEffect(() => {
  ...
  const act = async (action: AdminAction, options: ActionOptions) => {
    ...
    setBusy(true); setError(""); setNotice(""); setActionFeedback(null);
  ...
  return <section className="admin-page">
    ...
    {section !== "capabilities" && notice && <Feedback kind="success"><CheckCircle2 size={18} /><span>{notice}</span></Feedback>}
    {!snapshot && !error ? <div className="admin-loading" role="status"><Activity className="spinner" size={18} /><span>{c.loading}</span></div> : snapshot ? <>
      ...
      {section === "capabilities" && <CapabilitiesSection snapshot={snapshot} busy={busy} locale={locale} act={act} feedback={actionFeedback} />}
...
function CapabilitiesSection({ snapshot, busy, locale, act, feedback }: SectionProps) {
  const c = copy(locale);
  ...
  return <div className="admin-section-stack"><SectionTitle icon={<Settings2 />} title={c.capabilitiesTitle} body={c.capabilitiesBody} />
    <div className="admin-grid capability-grid">
      <CapabilityCard item={find("legacy-office")} title={c.officeCapability} body={c.officeCapabilityBody} icon={<FileType2 size={18} />} busy={busy} locale={locale} readOnly={snapshot.configurationReadOnly} act={act} feedback={feedback?.target === "legacy-office" ? feedback : null} />
      <CapabilityCard item={find("ocr")} title={c.ocrCapability} body={c.ocrCapabilityBody} icon={<ScanText size={18} />} busy={busy} locale={locale} readOnly={snapshot.configurationReadOnly} act={act} feedback={feedback?.target === "ocr" ? feedback : null} />
      <article className="card admin-entity-card speech-capability-card"><div className="entity-card-heading"><div className="entity-icon"><Speech size={18} /></div><div><h3>{c.speechCapability}</h3><p>{c.speechCapabilityBody}</p></div></div>
        <CapabilityControl item={find("transcription")} label={c.transcription} manageLocal busy={busy} locale={locale} readOnly={snapshot.configurationReadOnly} act={act} feedback={feedback && ["transcription", "media"].includes(feedback.target) ? feedback : null} />
        <CapabilityControl item={find("diarization")} label={c.diarization} manageLocal={false} busy={busy} locale={locale} readOnly={snapshot.configurationReadOnly} act={act} feedback={feedback?.target === "diarization" ? feedback : null} />
        <details className="admin-advanced"><summary>{c.technical}</summary><dl className="admin-detail-list"><Detail label={c.version} value={find("transcription").localVersion ?? "—"} /></dl>{find("transcription").localStatus !== "not-installed" && <button className="danger" data-capability-target="media" disabled={busy || snapshot.configurationReadOnly} type="button" onClick={() => void act({ schemaVersion: 1, action: "capability.remove", scope: "global", target: "media" }, { dangerous: true, confirm: c.capabilityRemoveConfirm, success: c.capabilityRemoveSuccess })}>{c.remove}</button>}</details>
      </article>
    </div>
  </div>;
}

function CapabilityCard({ item, title, body, icon, busy, locale, readOnly, act, feedback }: { item: CapabilityAdmin; title: string; body: string; icon: ReactNode; busy: boolean; locale: Locale; readOnly: boolean; act: SectionProps["act"]; feedback?: ActionFeedback | null }) {
  ...
  return <article className="card admin-entity-card"><div className="entity-card-heading"><div className="entity-icon">{icon}</div><div><h3>{title}</h3><p>{body}</p></div><StatusBadge tone={ready ? "ok" : item.status === "corrupt" ? "warning" : "neutral"}>{friendlyCapabilityStatus(item.status, locale)}</StatusBadge></div>
    <CapabilityControl item={item} busy={busy} locale={locale} readOnly={readOnly} act={act} feedback={feedback ?? null} />
    <details className="admin-advanced"><summary>{c.technical}</summary><dl className="admin-detail-list"><Detail label="ID" value={item.id} /><Detail label={c.version} value={item.localVersion ?? "—"} /></dl>{localInstalled && <button className="danger" data-capability-target={item.id} disabled={busy || readOnly} type="button" onClick={() => void act({ schemaVersion: 1, action: "capability.remove", scope: "global", target: item.id }, { dangerous: true, confirm: c.capabilityRemoveConfirm, success: c.capabilityRemoveSuccess })}>{c.remove}</button>}</details>
  </article>;
}

function CapabilityControl({ item, label, manageLocal = true, busy, locale, readOnly, act, feedback }: { item: CapabilityAdmin; label?: string; manageLocal?: boolean; busy: boolean; locale: Locale; readOnly: boolean; act: SectionProps["act"]; feedback?: ActionFeedback | null }) {
  ...
  return <div className="capability-control">{label && <h4>{label}</h4>}<Field label={c.currentSource}><select value={source} disabled={busy || readOnly} onChange={(event) => setSource(event.target.value)}>{item.sources.map((value) => <option key={value} value={value} disabled={value.startsWith("plugin:") && item.localStatus === "not-installed"}>{friendlyCapabilitySource(value, locale)}</option>)}</select></Field>
    <div className="admin-form-actions"><button data-capability-target={item.id} disabled={busy || readOnly || source === item.currentSource} type="button" onClick={() => void act({ schemaVersion: 1, action: "capability.use", scope: "project", target: item.id, source }, { dangerous: true, success: c.capabilitySourceSuccess })}>{c.useSource}</button>{manageLocal && (localReady ? <button className="secondary" data-capability-target={item.id} disabled={busy} type="button" onClick={() => void act({ schemaVersion: 1, action: "capability.verify", target: item.id }, { success: c.capabilityVerifySuccess })}>{c.verify}</button> : <button data-capability-target={item.id} disabled={busy || readOnly} type="button" onClick={() => void act({ schemaVersion: 1, action: "capability.install", target: item.id }, { dangerous: true, network: true, success: c.capabilityInstallSuccess })}><Download size={17} />{needsRepair ? c.repair : c.install}</button>)}</div>
...
function ProvidersSection({ snapshot, busy, locale, act }: SectionProps) {
  ...
  return <div className="admin-section-stack"><SectionTitle icon={<Cloud />} title={c.providersTitle} body={c.providersBody} action={<button type="button" disabled={snapshot.configurationReadOnly} onClick={() => setOpen((value) => !value)}><Plus size={17} />{open ? c.cancel : c.addProvider}</button>} />
    {open && <article className="card admin-tool-card"><h2>{c.addProvider}</h2><div className="admin-form-grid"><Field label={c.serviceName}><input value={name} maxLength={128} onChange={(event) => setName(event.target.value)} /></Field><Field label={c.baseUrl}><input value={url} maxLength={4096} placeholder="https://dashscope.aliyuncs.com/compatible-mode/v1" onChange={(event) => setUrl(event.target.value)} /></Field><Field label={c.model}><input value={model} maxLength={512} onChange={(event) => setModel(event.target.value)} /></Field><Field label={c.apiKeyEnv} hint={c.apiKeyHint}><input value={env} maxLength={128} placeholder="DASHSCOPE_API_KEY" onChange={(event) => setEnv(event.target.value)} /></Field><Field label={c.ocrModel} hint={c.modelMappingHint}><input value={ocrModel} maxLength={512} onChange={(event) => setOcrModel(event.target.value)} /></Field><Field label={c.transcriptionModel} hint={c.modelMappingHint}><input value={transcriptionModel} maxLength={512} onChange={(event) => setTranscriptionModel(event.target.value)} /></Field></div>
      ...
      <div className="admin-form-actions"><button disabled={busy || !name || !url || !model || !env || !validTimeout} type="button" onClick={() => void act({ schemaVersion: 1, action: "provider.add", scope, target: name, source: url, providerType: "openai-compatible", model, models: { ...(ocrModel ? { "vision-ocr": ocrModel } : {}), ...(transcriptionModel ? { "audio-transcription": transcriptionModel } : {}) }, apiKeyEnv: env, capabilities: providerCapabilities, ...(timeout ? { timeoutMs: Number(timeout) } : {}) }, { dangerous: true, success: c.providerAddedSuccess })}>{c.add}</button></div></article>}
...
function ProviderCard({ item, all, busy, locale, hosts, privateNetwork, readOnly, act }: { item: ProviderAdmin; all: ProviderAdmin[]; busy: boolean; locale: Locale; hosts: string; privateNetwork: boolean; readOnly: boolean; act: SectionProps["act"] }) {
  ...
  return <article className="card admin-entity-card"><div className="entity-card-heading"><div className="entity-icon"><Cloud size={18} /></div><div><h3>{item.name}</h3><p>{item.model ?? c.inherited}</p></div>{item.default ? <StatusBadge tone="ok">{c.defaultModel}</StatusBadge> : <StatusBadge tone={item.environmentSet === false ? "warning" : "neutral"}>{item.environmentSet === false ? c.environmentMissing : c.environmentReady}</StatusBadge>}</div>
    ...
    <details className="admin-advanced"><summary>{c.technical}</summary><dl className="admin-detail-list"><Detail label={c.apiKeyEnv} value={item.apiKeyEnv ?? c.inherited} /><Detail label={c.timeout} value={item.timeoutMs ? String(item.timeoutMs) : c.inherited} />{Object.entries(item.models).map(([capability, mappedModel]) => <Detail key={capability} label={`${c.model} · ${capability}`} value={mappedModel} />)}</dl>{layers.length > 0 && <ul className="admin-layer-list">{layers.map((layer) => <li key={layer.scope}><span>{friendlyScope(layer.scope, locale)}</span><button className="danger" disabled={busy || readOnly} type="button" onClick={() => void act({ schemaVersion: 1, action: "provider.remove", scope: layer.actionScope, target: item.name }, { dangerous: true, confirm: c.providerRemoveConfirm, success: c.success })}>{c.remove}</button></li>)}</ul>}</details>
  </article>;
}
...
function friendlyCapabilitySource(value: string, locale: Locale) { const c = copy(locale); if (value === "off") return c.off; const [, identity = value] = value.split(":", 2); const name = identity.split("/", 1)[0]; return value.startsWith("plugin:") ? `${c.localPlugin} · ${name}` : `${c.remoteProvider} · ${name}`; }
