...
function CapabilityInstallDialog({ api, item, busy, locale, act, onClose }: { api: ApiClient; item: CapabilityAdmin; busy: boolean; locale: Locale; act: SectionProps["act"]; onClose: () => void }) {
  ...
  const installLocal = async () => {
    ...
    try {
      ...
      const expectedPlugin = "official.media.whisper";
...
function doctorRemediationKey(item: DoctorAdmin) {
  ...
  if (id === "runtime.ocr" || id.includes("official.ocr.ppocrv6")) return "core:ocr";
...
function friendlyCapabilitySource(value: string, locale: Locale) { const c = copy(locale); if (value === "off") return c.off; if (value === "core:ocr") return locale === "zh-CN" ? "内置 OCR" : "Built-in OCR"; const [, identity = value] = value.split(":", 2); const name = identity.split("/", 1)[0] ?? identity; return value.startsWith("plugin:") ? `${c.localPlugin} · ${pluginDisplayName(name, locale)}` : `${c.remoteProvider} · ${name}`; }
...
function pluginDisplayName(id: string, locale: Locale) { const chinese = locale === "zh-CN"; const names: Record<string, [string, string]> = { "official.ocr.ppocrv6": ["本地 OCR（PP-OCR）", "Local OCR (PP-OCR)"], "official.media.whisper": ["本地语音（Whisper）", "Local speech (Whisper)"] }; return names[id]?.[chinese ? 0 : 1] ?? id; }
function pluginDescription(id: string, locale: Locale) { const chinese = locale === "zh-CN"; if (id.includes("ocr")) return chinese ? "识别扫描 PDF 和图片中的文字" : "Recognizes text in scans and images"; if (id.includes("media") || id.includes("whisper")) return chinese ? "语音转写与说话人识别" : "Speech transcription and speaker identification"; return chinese ? "本地扩展能力" : "Local extension capability"; }
function doctorInfo(item: DoctorAdmin, locale: Locale) {
  ...
  if (id === "runtime.ocr") return { title: chinese ? "内置 OCR" : "Built-in OCR", impact: chinese ? "当前 Core 中的 OCR 运行资源不可用。" : "The OCR runtime bundled with Core is unavailable.", action: chinese ? "重新下载或替换当前平台的 into-md，然后在页面顶部重新检查。" : "Download or replace into-md for this platform, then run the check again at the top of the page.", href: undefined, actionLabel: undefined };
  if (["runtime.asr", "runtime.diarization"].includes(id)) { const title = id === "runtime.asr" ? (chinese ? "语音转写能力" : "Transcription capability") : (chinese ? "说话人识别能力" : "Speaker identification capability"); return { title, impact: chinese ? "对应的本地转换功能无法使用。" : "The related local conversion feature is unavailable.", action: chinese ? "选择可用来源，或安装、修复对应的本地扩展。" : "Choose an available source, or install or repair its local extension.", href: capabilityHref, actionLabel: capabilityAction }; }
