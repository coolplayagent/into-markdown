...
export function HistoryPanel({ tasks, fallbackName, onOpen, onCleanup }: { tasks: TaskRecord[]; fallbackName: string; onOpen: (id: string) => void; onCleanup?: () => void }) {
  ...
  return <aside className="history-rail recent-history" aria-labelledby="history-rail-title">
    ...
    <footer className="history-rail-footer"><span>{filtered.length} {t("tasks")} · {safePage + 1}/{pages}</span><div>{onCleanup && <button className="history-cleanup secondary danger" type="button" aria-label={t("cleanup")} onClick={onCleanup}><Trash2 size={14} />{t("cleanup")}</button>}<span className="history-page-buttons"><button className="icon-button neutral" type="button" aria-label={t("previousPage")} disabled={safePage === 0} onClick={() => setPage((value) => Math.max(0, value - 1))}><ChevronLeft size={16} /></button><button className="icon-button neutral" type="button" aria-label={t("nextPage")} disabled={safePage + 1 >= pages} onClick={() => setPage((value) => Math.min(pages - 1, value + 1))}><ChevronRight size={16} /></button></span></div></footer>
  </aside>;
}
