...
`dlopen` 前安装并复核有限的 `RLIMIT_AS`，并由启动前创建的独立监督线程通过
`getrusage(RUSAGE_SELF)` 按固定周期监督 physical high-water mark，超限立即 `_exit`；
外层 Linux provider 同样由 Core 通过内核 `/proc/<pid>/status` 的 `VmHWM`/`VmRSS` 监督。
macOS 由父进程按固定周期监督 worker 的 physical footprint，超限立即 kill/reap；Windows
进程以 suspended 状态创建，加入带 process-memory hard limit 与 kill-on-close 的 Job Object
后才恢复。清单中的
