...
fn main() {
    ...
    let runtime_dispatch = cfg!(feature = "runtime-dispatch")
    ...
    if !whisper_root.exists() {
        ...
        fs_extra::dir::copy("./whisper.cpp", &out, &Default::default()).unwrap_or_else(|e| {
            panic!("Failed to copy whisper sources into {}: {}", whisper_root.display(), e)
        });
    }
    ...
    // A statically linked baseline is portable but leaves most x86_64 machines
    // several times slower than the native build.  GGML's dynamic CPU backend
    // authority builds a baseline plus optimized variants and selects the best
    // compatible one from actual CPUID/OSXSAVE results at runtime.  Keep the
    // portable x64 fallback and never inherit build-host features.
    if runtime_dispatch {
    ...
    if runtime_dispatch {
        // The installed shared import libraries are the complete link closure.
        // Recursing through every all-variants CMake intermediate directory makes
        // Windows rustc command lines exceed their practical length limit.
        println!("cargo:rustc-link-search=native={}", destination.join("lib").display());
    } else {
        add_link_search_path(&out.join("build")).unwrap();
        println!("cargo:rustc-link-search=native={}", destination.display());
    }
    if runtime_dispatch || cfg!(feature = "intel-sycl") {
