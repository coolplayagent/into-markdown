...
#[cfg(all(test, feature = "nightly-tests"))]
mod test {
    use super::*;
    ...
    pub fn assert_stereo_to_mono_err() {
        ...
        assert!(
            match result {
                Err(WhisperError::InputOutputLengthMismatch { input_len, output_len }) => {
                    assert_eq!(
                        input_len,
                        samples.len() / 2,
    ...
    pub fn bench_stereo_to_mono(b: &mut test::Bencher) {
        ...
        let mut output = vec![0.0; samples.len() / 2];
        b.iter(|| {
            black_box(convert_stereo_to_mono_audio(black_box(&samples), black_box(&mut output)))
        });
    }
    ...
    pub fn bench_integer_to_float(b: &mut test::Bencher) {
        ...
        let mut output = vec![0.0f32; samples.len()];
        b.iter(|| {
            black_box(convert_integer_to_float_audio(black_box(&samples), black_box(&mut output)))
        });
    }
}
