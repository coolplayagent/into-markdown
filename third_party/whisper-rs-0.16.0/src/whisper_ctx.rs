...
fn path_to_bytes(path: &Path) -> Result<Cow<'_, [u8]>, WhisperError> {
    path.to_str()
        ...
        .ok_or_else(|| WhisperError::InvalidUtf8 { error_len: None, valid_up_to: 0 })
}
...
impl<'a> WhisperContextParameters<'a> {
    ...
    fn to_c_struct(&self) -> whisper_rs_sys::whisper_context_params {
        ...
        let mut dtw_aheads = whisper_rs_sys::whisper_aheads { n_heads: 0, heads: std::ptr::null() };
...
pub enum DtwMode<'a> {
    ...
    Custom { aheads: &'a [whisper_rs_sys::whisper_ahead] },
