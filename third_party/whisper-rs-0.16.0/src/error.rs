...
impl std::fmt::Display for WhisperError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        match self {
            ...
            InvalidUtf8 { valid_up_to, error_len: None } => write!(
                f,
                "Invalid UTF-8 detected in a string from Whisper. Index: {}.",
                valid_up_to
            ),
            NullByteInString { idx } => {
                write!(f, "A null byte was detected in a user-provided string. Index: {}", idx)
            }
            NullPointer => write!(f, "Whisper returned a null pointer."),
            InvalidText => write!(f, "Whisper failed to convert the provided text into tokens."),
            ...
            InputOutputLengthMismatch { output_len, input_len } => {
                write!(
                    f,
