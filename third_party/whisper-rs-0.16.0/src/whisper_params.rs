...
type ProgressCallbackFn = Box<dyn FnMut(i32) + Send>;
type AbortCallbackFn = Box<dyn FnMut() -> bool + Send>;
...
pub struct FullParams<'a, 'b> {
    ...
    progress_callback_safe: Option<Arc<Mutex<ProgressCallbackFn>>>,
    abort_callback_safe: Option<Arc<Mutex<AbortCallbackFn>>>,
...
impl<'a, 'b> FullParams<'a, 'b> {
        ...
        O: Into<Option<F>>,
    {
        ...
        unsafe extern "C" fn trampoline(
            ...
            let callback = &*(user_data as *const Mutex<ProgressCallbackFn>);
        ...
        O: Into<Option<F>>,
    {
        ...
        unsafe extern "C" fn trampoline(user_data: *mut c_void) -> bool {
            let callback = &*(user_data as *const Mutex<AbortCallbackFn>);
            let mut callback = callback.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
            callback()
        }
        ...
        match closure.into() {
            Some(closure) => {
                let callback: Arc<Mutex<AbortCallbackFn>> = Arc::new(Mutex::new(Box::new(closure)));
                self.fp.abort_callback = Some(trampoline);
                self.fp.abort_callback_user_data =
                    Arc::as_ptr(&callback).cast_mut().cast::<c_void>();
                self.abort_callback_safe = Some(callback);
            }
            None => {
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
        f.debug_struct("FullParams")
            ...
            .field(
                ...
                &self.progress_callback_safe.as_ref().map_or("None", |_| "Some"),
            )
            .field(
                "abort_callback_safe",
            ...
            .field(
                ...
                &self.segment_calllback_safe.as_ref().map_or("None", |_| "Some"),
            )
            .finish()
    }
}
...
mod callback_ownership_tests {
    ...
    fn configured(counter: &Arc<AtomicUsize>) -> FullParams<'static, 'static> {
        let abort_counter = DropCounter(Arc::clone(counter));
        let progress_counter = DropCounter(Arc::clone(counter));
        let mut params = FullParams::new(SamplingStrategy::Greedy { best_of: 1 });
        params.set_abort_callback_safe(move || {
            let _counter = &abort_counter;
            true
        });
        params.set_progress_callback_safe(move |_| {
            let _counter = &progress_counter;
        });
        params
    }

    fn configured_error(counter: &Arc<AtomicUsize>) -> Result<(), ()> {
        let _params = configured(counter);
        Err(())
    }

    #[test]
    fn callbacks_drop_after_repeated_normal_cancel_and_error_paths() {
        let drops = Arc::new(AtomicUsize::new(0));
        for iteration in 0..128 {
            let normal = configured(&drops);
            let progress = normal.fp.progress_callback.unwrap();
            // SAFETY: user data is owned by `normal`, remains live for this call, and
            // the callback receives the exact arguments promised by whisper.cpp.
            unsafe {
                progress(
                    std::ptr::null_mut(),
                    std::ptr::null_mut(),
                    50,
                    normal.fp.progress_callback_user_data,
                );
            }
            drop(normal);

            let cancelled = configured(&drops);
            let abort = cancelled.fp.abort_callback.unwrap();
            // SAFETY: user data is owned by `cancelled` and the call is completed
            // before the params (and therefore the callback allocation) are dropped.
            assert!(unsafe { abort(cancelled.fp.abort_callback_user_data) });
            drop(cancelled);

            assert_eq!(configured_error(&drops), Err(()));
            assert_eq!(drops.load(Ordering::SeqCst), (iteration + 1) * 6);
        }
    }
}
...
mod test_whisper_params_initial_prompt {
    ...
    impl<'a, 'b> FullParams<'a, 'b> {
        pub fn get_initial_prompt(&self) -> &str {
            ...
            unsafe { std::ffi::CStr::from_ptr(self.fp.initial_prompt).to_str().unwrap() }
        }
    }
