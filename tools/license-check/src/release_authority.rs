...
const REVIEWED_AUTHORITY_SHA256: &str =
    "251327e97d357e481f8fc741b36d04f6b14b07d45935b029b47fb2a5fe6fd400";
    ...
    for profile in &authority.profiles {
        ...
        for (record, actual) in profile.files.iter().zip(files) {
            if record.bytes != actual.bytes || record.sha256 != actual.sha256 {
                errors.push(format!(
                    "release material profile {} drifted for {}; generated bytes={} sha256={}",
                    profile.request, record.path, actual.bytes, actual.sha256
                ));
            }
        }
    }
