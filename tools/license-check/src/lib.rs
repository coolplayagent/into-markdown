...
mod ffmpeg;
mod materials;
use materials::validate_release_file_collection;
mod models_fixtures;
mod native;
mod npm;
pub mod release;
...
fn validate_fixture_corpus(
    ...
    for format in required_formats {
        // Legacy Office worker fixtures, EPUB and image adversarial fixtures, plus PDF and ZIP fixtures are
        // constructed byte-for-byte in their converter/process/API test modules. This keeps binaries out
        ...
        if matches!(format, "doc" | "epub" | "image" | "pdf" | "ppt" | "xls" | "zip") {
            continue;
        }
...
fn ort_target_audit_is_valid(target: &str, asset: &OrtTarget) -> bool {
    is_safe_model_file_name(&asset.asset)
        ...
        && (256 * 1024 * 1024..=8 * 1024 * 1024 * 1024)
            .contains(&asset.worker_physical_memory_overhead_bytes)
