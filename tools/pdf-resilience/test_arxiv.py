...
class OcrProcessingQualityTests(unittest.TestCase):
    ...
    def test_successful_delivery_cannot_hide_document_ocr_failure_rate(self):
        ...
        report = self.sample(100, 95)
        report['cases'][0]['items'] = [dict(diagnostics=[dict(
            code='pdf.scannedPage', locator=dict(page=p)) for p in range(1, 101)])]
        self.assertTrue(completion.audit(report)['processingQualityPassed'])
    ...
    def test_resource_skips_cannot_inflate_completion_rate(self):
        ...
        report['cases'][0]['items'][0]['diagnostics'][0]['code'] = 'pdf.optionalOcrSkipped'
        self.assertFalse(completion.audit(report)['processingQualityPassed'])
